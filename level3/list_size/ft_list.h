...
{
    struct s_list *next; // Puntero que guarda la dirección del siguiente nodo en la lista
                          // Este campo permite "enlazar" un nodo con otro, creando la lista
                          // Si 'next' es NULL, significa que este es el último nodo de la lista

    void *data;         // Puntero a los datos almacenados en el nodo
                          // El tipo 'void *' significa que puede apuntar a cualquier tipo de dato
                          // Esto hace que la lista sea flexible y pueda almacenar cualquier tipo de información
                          // (por ejemplo, enteros, cadenas, estructuras complejas, etc.)

} t_list;              // 't_list' es el alias para esta estructura, que representa un nodo en la lista
						//El prefijo t_ es una convención en C que indica que el nombre representa un tipo definido por el programador (de "type"). Este prefijo ayuda a diferenciarlo de otras cosas en tu código, como variables, funciones o macros. Ejemplo:
						...
						//Claridad: Cuando ves t_ en el código, sabes rápidamente que es un alias de tipo, no una variable o función.
